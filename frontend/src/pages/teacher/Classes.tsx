...
//   // };
//   const handleMarkAttendance = (classData: TeacherClassData) => {
//   // Use classroom_id if available, otherwise fall back to section_id
//   const classroomId = classData.classroom_id || classData.section_id;
//   console.log('🎯 Navigating to attendance with classroom ID:', classroomId);
...
//   const handleViewStudents = (classData: TeacherClassData) => {
//   // Use classroom_id if available, otherwise fall back to section_id
//   const classroomId = classData.classroom_id || classData.section_id;
//   console.log('🎯 Navigating to students with classroom ID:', classroomId);
//   navigate(`/teacher/students/${classroomId}`);
// };

// const handleSendMessage = (classData: TeacherClassData) => {
//   // Use classroom_id if available, otherwise fall back to section_id
//   const classroomId = classData.classroom_id || classData.section_id;
//   console.log('🎯 Navigating to messages with classroom ID:', classroomId);
//   navigate(`/teacher/messages/class/${classroomId}`);
// };
...
interface TeacherClassData {
  ...
  room_number: string;
  is_class_teacher: boolean;
  classroom_id?: number; // The actual classroom ID for API calls
  all_subjects?: any[]; // Added for displaying individual subjects
}
        ...
        {selectedClass && (
          <div className="bg-white dark:bg-slate-800 rounded-xl p-6 shadow-sm border border-slate-200 dark:border-slate-700">
            <div className="flex items-center justify-between mb-6">
              ...
              <div className="flex space-x-2">
                <button
                  onClick={() => handleSendMessage(selectedClass)}
                  className="bg-orange-600 hover:bg-orange-700 text-white px-4 py-2 rounded-lg flex items-center space-x-2 transition-colors duration-200"
                >
                  <MessageSquare className="w-4 h-4" />
                  <span>Send Message</span>
