...
class SettingsService {
  ...
  async updateCommunicationSettings(settings: Partial<CommunicationSettings>): Promise<CommunicationSettings> {
    try {
      const response = await api.put('communication-settings/', settings);
      return response.data;
    } catch (error) {
  ...
  async uploadLogo(file: File): Promise<{ logoUrl: string }> {
    try {
      ...
      const response = await fetch(`${import.meta.env.VITE_API_URL || 'http://localhost:8000/api'}/school-settings/upload-logo/`, {
        method: 'POST',
        headers: {
          'Authorization': `Token ${localStorage.getItem('authToken')}`,
        },
  ...
  async uploadFavicon(file: File): Promise<{ faviconUrl: string }> {
    try {
      ...
      const response = await fetch(`${import.meta.env.VITE_API_URL || 'http://localhost:8000/api'}/school-settings/upload-favicon/`, {
        method: 'POST',
        headers: {
          'Authorization': `Token ${localStorage.getItem('authToken')}`,
        },
