...
const ResultRecordingForm = ({
  ...
  const setupEditResult = async () => {
    try {
      ...
      // Get education level from teacher assignments (source of truth), not from editResult
      let normalizedLevel = '';
      let actualEducationLevel = '';
      
      if (subjectIdStr) {
        const subjectAssignments = teacherAssignments.filter(a => a.subject_id === parseInt(subjectIdStr));
        
        if (subjectAssignments.length > 0) {
          // Use the education level from the teacher's assignment as the source of truth
          actualEducationLevel = subjectAssignments[0].education_level;
          normalizedLevel = (actualEducationLevel || '')
            .toString()
            .replace(/_/g, ' ')
            .toLowerCase()
            .trim();
          
          console.log('📝 Education level from assignment:', actualEducationLevel, '→', normalizedLevel);
          setSelectedEducationLevel(normalizedLevel);
          
          const upperLevel = (actualEducationLevel || '')
            .toString()
            .replace(/\s+/g, '_')
            .toUpperCase();
          const configForLevel = scoringConfigs.find((c: any) => c.education_level === upperLevel && (c.is_default || c.is_active));
          ...
          console.log('📝 Using education level:', upperLevel, 'Config:', configForLevel ? 'Found' : 'Not found');
        }
      ...
      // Now use the ACTUAL education level from assignments to determine field structure
      const educationLevelUpper = (actualEducationLevel || '')
        .toString()
        .replace(/\s+/g, '_')
        .toUpperCase();
      
      console.log('📝 Determining field structure for:', educationLevelUpper);
      ...
      // Use education level from assignment, NOT from stored result
      if (educationLevelUpper.includes('SENIOR')) {
        console.log('📝 Loading SENIOR SECONDARY scores');
        setAssessmentScores({
          test1: (editResult.first_test_score ?? editResult.test1 ?? 0).toString(),
          test2: (editResult.second_test_score ?? editResult.test2 ?? 0).toString(), 
          test3: (editResult.third_test_score ?? editResult.test3 ?? 0).toString(),
      ...
      } else if (educationLevelUpper.includes('NURSERY')) {
        console.log('📝 Loading NURSERY scores');
        setAssessmentScores({
          max_marks: (editResult.max_marks ?? 100).toString(),
          mark_obtained: (editResult.mark_obtained ?? editResult.total_score ?? editResult.ca_score ?? 0).toString(),
          remarks: extractedRemarks
        });
      } else if (educationLevelUpper.includes('PRIMARY') || educationLevelUpper.includes('JUNIOR')) {
        console.log('📝 Loading PRIMARY/JUNIOR scores');
        setAssessmentScores({
          ca_score: (editResult.ca_score ?? editResult.continuous_assessment_score ?? 0).toString(),
          take_home_marks: (editResult.take_home_marks ?? editResult.take_home_score ?? 0).toString(),
          take_home_test: (editResult.take_home_test ?? editResult.take_home_test_score ?? 0).toString(),
