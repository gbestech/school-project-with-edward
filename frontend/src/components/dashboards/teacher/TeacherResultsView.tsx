...
const TeacherResults: React.FC = () => {
  ...
  async function loadTeacherData() {
    try {
      ...
      if (subjectIds.length) {
        try {
          // Try approach 1: Filter by subject IDs
          resultsData = await ResultService.getStudentResults({ subject: subjectIds.join(',') });
          debugLog += `Results from subject filter: ${Array.isArray(resultsData) ? resultsData.length : 'not array'}\n`;
        } catch (err) {
          debugLog += `Error with subject filter: ${err}\n`;
          
          // Try approach 2: Get all results and filter client-side
          try {
            resultsData = await ResultService.getStudentResults({});
            debugLog += `Results from no filter: ${Array.isArray(resultsData) ? resultsData.length : 'not array'}\n`;
          } catch (err2) {
            debugLog += `Error with no filter: ${err2}\n`;
          }
        }
