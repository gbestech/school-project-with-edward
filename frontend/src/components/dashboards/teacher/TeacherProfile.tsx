...
const TeacherProfile: React.FC<TeacherProfileProps> = ({ onRefresh }) => {
  ...
  // Helper functions for accurate counts
  const getUniqueStudentCount = () => {
    if (!profileData?.classroom_assignments) return 0;
    
    // Get all unique student IDs across all classroom assignments
    const studentIds = new Set();
    profileData.classroom_assignments.forEach((assignment: any) => {
      if (assignment.classroom?.students) {
        assignment.classroom.students.forEach((student: any) => {
  ...
  const getClassroomCount = () => {
    if (!profileData?.classroom_assignments) return 0;
    // Count unique classrooms
    const classroomIds = new Set();
    profileData.classroom_assignments.forEach((assignment: any) => {
      if (assignment.classroom?.id) {
  ...
  const getSubjectCount = () => {
    if (!profileData?.assigned_subjects) return 0;
    return profileData.assigned_subjects.length;
  };
                  ...
                  <div className="space-y-3">
                    ...
                    <div className="flex items-center justify-between p-3 bg-blue-50 dark:bg-blue-900/20 rounded-xl">
                      <div className="flex items-center gap-3">
                        ...
                        <span className="text-slate-700 dark:text-slate-300 font-medium text-sm">Classrooms</span>
                      </div>
                      <span className="text-xl font-bold text-blue-600 dark:text-blue-400">
                        {getClassroomCount()}
                      </span>
                    ...
                    <div className="flex items-center justify-between p-3 bg-blue-50 dark:bg-blue-900/20 rounded-xl">
                      <div className="flex items-center gap-3">
                        ...
                        <span className="text-slate-700 dark:text-slate-300 font-medium text-sm">Total Students</span>
                      </div>
                      <span className="text-xl font-bold text-blue-600 dark:text-blue-400">
