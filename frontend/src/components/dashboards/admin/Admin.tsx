...
const AdminDashboard: React.FC<AdminDashboardProps> = ({
  ...
  // ✅ Build navigation items with proper permission checks
  const buildNavigationItems = () => {
    ...
    // ✅ Admins - ONLY visible to superadmin and admin roles
    if (canViewAdminList()) {
      items.push({ name: 'Admins', icon: User, path: '/admin/admins' });
    }

    // ✅ Password Recovery - ONLY visible to superadmin and admin roles
    ...
    // ✅ Settings - ONLY visible to superadmin and admin roles
    if (canViewSettings()) {
  ...
  return (
    ...
    >
      <div className="flex h-screen">
        ...
        {/* ✅ Floating Action Button for Password Recovery - Only show if user has permission */}
        {canAccessPasswordRecovery() && (
          <button
            onClick={() => navigate('/admin/password-recovery')}
            className={`
