...
const LessonAttendanceDashboard = () => {
  ...
  // Fetch lesson attendance from backend
  useEffect(() => {
    setLoading(true);
    setError(null);
    getLessonAttendance({ date: filters.date })
      .then((data) => {
        // Map backend data to LessonAttendanceRecord[]
        const mapped: LessonAttendanceRecord[] = (data.results || data).map((rec: AttendanceRecordBackend) => ({
          id: rec.id,
          name: rec.student ? `Student ID ${rec.student}` : rec.teacher ? `Teacher ID ${rec.teacher}` : 'Unknown',
  ...
  const handleEdit = (record: LessonAttendanceRecord): void => {
  ...
  const handleSave = async (formData: Omit<LessonAttendanceRecord, 'id' | 'date'> & { date?: string }): Promise<void> => {
    setLoading(true);
    setError(null);
    try {
      if (editingRecord) {
        ...
        setAttendanceRecords(prev => prev.map(record => record.id === editingRecord.id ? { ...record, ...formData } : record));
      } else {
        // Add new record
