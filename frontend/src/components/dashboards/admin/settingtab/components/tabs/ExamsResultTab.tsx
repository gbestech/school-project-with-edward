...
const mockService = {
  ...
  async getAssessmentTypes() { return []; },
  async getExamSessions() { return []; },
  async getScoringConfigurations() { return []; },
  async getAcademicSessions() { return []; },
  async createGrade(data: any) { return { id: Date.now().toString(), ...data }; },
  async updateGrade(id: string, data: any) { return { id, ...data }; },
  async deleteGrade(id: string) { return; },
  async deleteGradingSystem(id: string) { return; }
};
...
const ExamsResultTab: React.FC = () => {
  const [activeSections, setActiveSections] = useState<Set<string>>(new Set(['grading-systems']));
  ...
  const [gradeForm, setGradeForm] = useState({
    ...
    min_score: 0,
    max_score: 0,
    grade_point: undefined as number | undefined,
    description: '',
    is_passing: true
  });
  ...
  const loadData = async () => {
    try {
      setLoading(true);
      const [gradingSystemsData, gradesData] = await Promise.all([
        mockService.getGradingSystems(),
        mockService.getGrades()
      ]);
  ...
  const handleDeleteGradingSystem = async (id: string) => {
    if (window.confirm('Are you sure you want to delete this grading system?')) {
      try {
        await mockService.deleteGradingSystem(id);
        toast.success('Grading system deleted successfully');
        loadData();
      } catch (error) {
        console.error('Error deleting grading system:', error);
  ...
  const handleCreateGrade = async () => {
    try {
      setSaving(true);
      const newGrade = await mockService.createGrade(gradeForm);
  ...
  const handleUpdateGrade = async (id: string) => {
    try {
      setSaving(true);
      const updatedGrade = await mockService.updateGrade(id, gradeForm);
  ...
  const handleDeleteGrade = async (id: string) => {
    if (window.confirm('Are you sure you want to delete this grade?')) {
      try {
        await mockService.deleteGrade(id);
          ...
          <div className="flex items-center justify-between">
            <div className="flex items-center space-x-4">
              ...
              <div>
                <h1 className="text-3xl font-bold text-gray-900">Exams & Results Settings</h1>
                <p className="text-gray-600 mt-1">
                  Configure grading systems and manage grade ranges
                </p>
              </div>
            </div>
          </div>
