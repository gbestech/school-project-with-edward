...
const RolesPermissions = () => {
  ...
  const assignRoleToUser = async () => {
    ...
    try {
      ...
      const method = existingUserRole ? 'PUT' : 'POST';
      const url = existingUserRole 
        ? `${API_BASE_URL}/api/school-settings/user-roles/${existingUserRole.id}/`
        : `${API_BASE_URL}/api/school-settings/user-roles/`;
  ...
  const saveRolePermissions = async () => {
    ...
    try {
      ...
      // Use PATCH on the main role endpoint (based on your API routes)
      const payload = {
        name: editingRolePermissions.name,
        description: editingRolePermissions.description,
        color: editingRolePermissions.color,
