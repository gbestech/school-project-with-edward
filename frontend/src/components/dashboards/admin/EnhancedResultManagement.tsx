...
// const EnhancedResultManagement: React.FC<EnhancedResultRecordingProps> = () => {
...
import { useGlobalTheme } from '@/contexts/GlobalThemeContext';
import StudentService, { Student } from '@/services/StudentService';
import ResultService, { StandardResult } from '@/services/ResultService';
import EnhancedResultRecording from './EnhancedResultRecording';
import EditResultForm from './EditResultForm';
import { toast } from 'react-toastify';
import { useNavigate } from 'react-router-dom';
...
const EnhancedResultManagement: React.FC<EnhancedResultRecordingProps> = ({ onResultAdded }) => {
  ...
  const loadData = async () => {
    try {
      ...
      // Load students and all results separately
      const [studentsData] = await Promise.all([
        StudentService.getStudents()
      ]);
      
      // Get all results using the new getAllResults method
  ...
  const getStudentResults = (studentId: number) => {
    const studentResults = results.filter(result => {
      // Handle both cases: when student is an object with id, or when it's just the id
      const resultStudentId = typeof result.student === 'object' ? result.student.id : result.student;
      return resultStudentId === studentId.toString() || resultStudentId === studentId;
    });
                  ...
                  {studentResults.length > 0 && (
                    <div className="mb-4">
                      ...
                      <div className="space-y-1.5">
                        {studentResults.map((result) => (
                          <div key={result.id} className="flex items-center justify-between">
                            <span className="text-xs truncate max-w-[60%]" title={result.subject?.name || 'Unknown Subject'}>
                      ...
                      <tr key={student.id} className={`hover:${themeClasses.bgSecondary}`}>
                        ...
                        <td className="px-6 py-4 whitespace-nowrap">
                          <div className="space-y-1">
                            {studentResults.slice(0, 3).map((result) => (
                              <div key={result.id} className="flex items-center justify-between">
                                <span className="text-xs">{result.subject?.name || 'Unknown Subject'}</span>
                                {getStatusBadge(result.status)}
                              </div>
                            ))}
                            {studentResults.length > 3 && (
        ...
        <div className="fixed inset-0 bg-black bg-opacity-50 flex items-center justify-center z-50 p-4">
          <div className={`${themeClasses.bgCard} rounded-lg shadow-xl w-full max-w-4xl max-h-[90vh] overflow-y-auto`}>
            <div className="p-6">
              ...
              <EnhancedResultRecording onResultAdded={handleResultAdded} />
            </div>
          </div>
        </div>
      )}
