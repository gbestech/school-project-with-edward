...
const Navbar = () => {
  ...
  // Safe theme icon handling
  const ThemeIcon = themeIcons[theme] || Sun;

  // Safe user display name handling
  const getUserDisplayName = () => {
    if (!user) return '';
    return user.first_name || user.full_name || 'User';
  };
  ...
  return (
    <>
      ...
      <nav
        ...
        style={{
          background: scrolled ? 'var(--background-secondary)' : 'var(--background)',
      ...
      >
        ...
        <div style={{ height: 1, background: 'linear-gradient(to right, transparent, #60a5fa99, transparent)', opacity: 0.6 }} />
          ...
          <div className="flex justify-between items-center h-24">
            ...
            <Link to="/" className="flex items-center space-x-5 group">
              ...
              <div className="transform transition-all duration-500 group-hover:translate-x-1">
                <div className="text-3xl lg:text-4xl font-black bg-gradient-to-r from-gray-300 via-blue-500 to-purple-600 bg-clip-text text-transparent tracking-tight">
                 {settings?.school_name || "God's Treasure Schools"}
                </div>
                <div className="text-xs font-semibold tracking-[0.2em] uppercase mt-1 opacity-80 group-hover:opacity-100 transition-opacity duration-300"
                  style={{ color: 'var(--secondary-text)' }}>
                  {settings?.school_motto || "Premium Education"}
                </div>
              </div>
            </Link>
            ...
            <div className="hidden lg:flex items-center space-x-12">
              {navLinks.map((item, index) => (
                <Link
                  ...
                  className="relative font-medium text-lg transition-all duration-300 group py-2"
                  style={{
                ...
                >
                  ...
                  <div className="absolute inset-0 rounded-xl opacity-0 group-hover:opacity-100 transition-opacity duration-300 -z-10" style={{ background: 'var(--surface)' }}></div>
                </Link>
              ))}
            </div>
              ...
              {isAuthenticated && user ? (
                ...
                <div className="relative" data-menu>
                  ...
                  >
                    ...
                    <span className="text-slate-200/120 font-medium group-hover:text-white transition-colors duration-300">
                      {getUserDisplayName()}
                    </span>
                  ...
                  {isUserMenuOpen && (
                    <div className="absolute right-0 mt-2 w-56 bg-slate-900/95 backdrop-blur-3xl rounded-2xl border border-slate-800/50 shadow-2xl shadow-slate-900/20 overflow-hidden">
                      <div className="p-4 border-b border-slate-800/50 bg-gradient-to-r from-blue-500/10 to-purple-500/10">
                        ...
                        <p className="text-slate-300/70 text-sm capitalize">{user.role || 'User'}</p>
                      </div>
                      <div className="py-2">
                        <Link
