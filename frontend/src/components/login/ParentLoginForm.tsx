...
const ParentLoginForm: React.FC<LoginProps> = ({ 
  onLogin, 
  onBackToHome, 
  onSocialLogin,
  onForgotPassword,
  ...
  const [form, setForm] = useState<LoginCredentials>({
    username: '',
    password: '',
    role: (initialRole ?? 'parent') as UserRole,
    rememberMe: false
  });
  ...
  const handleLogin = async (e: React.FormEvent) => {
    ...
    try {
      ...
      console.log('Parent login payload:', form);
      await onLogin(form);
            ...
            <div className="space-y-6">
              <div className="space-y-4">
                <div className="inline-flex items-center space-x-2 bg-gradient-to-r from-indigo-500/20 to-purple-500/20 backdrop-blur-xl px-4 py-2 rounded-full border border-white/20">
                  ...
                  <span className="text-sm font-semibold text-indigo-200">Parent Portal</span>
                </div>
                  ...
                  <div className="inline-flex items-center space-x-2 bg-gradient-to-r from-indigo-500/20 to-purple-500/20 backdrop-blur-xl px-4 py-2 rounded-full border border-white/20 mb-4">
                    {loginSuccess ? (
                      ...
                      <>
                        ...
                        <span className="text-sm font-semibold">Parent Access</span>
                        <Star size={14} className="text-yellow-400" />
                      </>
                    )}
                  </div>
                  <h2 className="text-3xl font-black mb-2">
                    {loginSuccess ? 'Success!' : 'Parent Sign In'}
                  </h2>
