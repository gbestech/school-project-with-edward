...
function renderRichContent(content: any): string {
  ...
  if (processedContent.match(/^https?:\/\/.*\.(jpg|jpeg|png|gif|webp|svg|bmp)$/i)) {
    console.log('🖼️ Converting plain image URL to HTML:', processedContent.substring(0, 100));
    processedContent = `<img src="${processedContent}" alt="Question Image" style="max-width: 100%; height: auto; margin: 10px 0; display: block; border: 1px solid #ddd; border-radius: 4px; padding: 4px;" />`;
  }
  ...
  if (imgMatches) {
    console.log(`🖼️ Found ${imgMatches.length} img tags in content`);
    processedContent = processedContent.replace(
      /<img\s+([^>]*?)src="([^"]*)"([^>]*?)>/gi,
      '<img src="$2" alt="Question Image" style="max-width: 100%; height: auto; margin: 10px 0; display: block; border: 1px solid #ddd; border-radius: 4px; padding: 4px;" />'
    );
  ...
  // Handle tables - ensure proper styling (only if table tags exist)
  const hasTable = processedContent.includes('<table');
  if (hasTable) {
    console.log('📋 Processing table HTML in content');
    
    // Don't add duplicate styles if they already exist
    processedContent = processedContent.replace(
      /<table(?:\s+style="[^"]*")?([^>]*)>/gi,
      '<table style="border-collapse: collapse; width: 100%; margin: 10px 0; border: 1px solid #ddd;"$1>'
    );
...
export function generateExamHtml(
  ...
  // Count questions with images/tables for logging
  const imageCount = [
    ...(exam.objective_questions || []),
    ...(exam.theory_questions || []),
    ...(exam.practical_questions || [])
  ].filter((q: any) => q.image).length;

  const tableCount = [
    ...(exam.objective_questions || []),
    ...(exam.theory_questions || []),
    ...(exam.practical_questions || [])
  ].filter((q: any) => q.table).length;

  console.log(`📊 Exam contains ${imageCount} images and ${tableCount} tables`);
