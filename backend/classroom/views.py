...
from rest_framework.views import APIView
from django_filters.rest_framework import DjangoFilterBackend
from django.db.models import Q, Count, Prefetch, Min, Max, Avg, Sum
from django.utils.decorators import method_decorator
from django.views.decorators.cache import cache_page
from django.core.cache import cache
from django.db import transaction
...
class GradeLevelViewSet(AutoSectionFilterMixin, viewsets.ModelViewSet):
    ...
    def get_queryset(self):
        """Apply automatic section filtering while preserving mixin behavior"""
        return super().get_queryset().select_related().all()

    @action(detail=True, methods=["get"])
    def subjects(self, request, pk=None):
        grade = self.get_object()
        subjects = Subject.objects.filter(grade_levels=grade).prefetch_related(
            "grade_levels"
        )
        serializer = SubjectSerializer(subjects, many=True)
        return Response(serializer.data)
    ...
    def students(self, request, pk=None):
        grade = self.get_object()
        enrollments = StudentEnrollment.objects.filter(
            classroom__section__grade_level=grade, is_active=True
        ).select_related("student__user", "classroom")
        # import serializer lazily to avoid circular imports
        from students.serializers import StudentListSerializer
    ...
    def classrooms(self, request, pk=None):
        grade = self.get_object()
        classrooms = Classroom.objects.filter(section__grade_level=grade)
...
class SectionViewSet(AutoSectionFilterMixin, viewsets.ModelViewSet):
    ...
    def get_queryset(self):
        """Apply section filtering and keep grade_level eager-loaded"""
        return super().get_queryset().select_related("grade_level").all()
...
class TeacherViewSet(AutoSectionFilterMixin, viewsets.ModelViewSet):
    ...
    def get_queryset(self):
        """Preserve section filtering from mixin and eager load user"""
    ...
    def subjects(self, request, pk=None):
        teacher = self.get_object()
        assignments = (
            teacher.classroomteacherassignment_set.filter(is_active=True)
            .select_related("subject")
            .prefetch_related("subject__grade_levels")
        )
        subjects = [assignment.subject for assignment in assignments]
        serializer = SubjectSerializer(subjects, many=True)
        return Response(serializer.data)
...
class StudentViewSet(AutoSectionFilterMixin, viewsets.ModelViewSet):
    ...
    # choose serializer dynamically to avoid circular import issues
    def get_serializer_class(self):
    ...
    def get_queryset(self):
        """Return students scoped by sections via mixin and eager load user."""
        return super().get_queryset().select_related("user").all()

    @action(detail=True, methods=["get"])
    def current_class(self, request, pk=None):
        # fetch most recent active enrollment
        enroll = (
            StudentEnrollment.objects.filter(student_id=pk, is_active=True)
            .select_related(
                "classroom__section__grade_level", "classroom__class_teacher"
            )
            .order_by("-created_at")
            .first()
        )
        ...
        from students.serializers import StudentEnrollmentSerializer as SE_Serializer

        serializer = SE_Serializer(enroll)
        return Response(serializer.data)
    ...
    def subjects(self, request, pk=None):
        ...
        subject_qs = Subject.objects.filter(
            classroomteacherassignment__classroom__in=[
                e.classroom for e in enrollments
            ],
            classroomteacherassignment__is_active=True,
        ).distinct()
        serializer = SubjectListSerializer(subject_qs, many=True)
        return Response(serializer.data)
    ...
    def schedule(self, request, pk=None):
        ...
        schedules = ClassSchedule.objects.filter(
            classroom__in=[e.classroom for e in enrollments], is_active=True
    ...
    @action(detail=True, methods=["get"])
    def enrollment_history(self, request, pk=None):
        enrollments = StudentEnrollment.objects.filter(student_id=pk).select_related(
            "classroom", "classroom__section__grade_level"
        )
...
class SubjectViewSet(AutoSectionFilterMixin, viewsets.ModelViewSet):
    ...
    def get_queryset(self):
        return super().get_queryset().prefetch_related("grade_levels", "prerequisites")
    ...
    def by_category(self, request):
        ...
        if not category:
            return Response(
                {"error": "category query parameter is required"}, status=400
            )
    ...
    def by_education_level(self, request):
        level = request.query_params.get("level")
        if not level:
            return Response({"error": "level query parameter is required"}, status=400)
        qs = self.get_queryset().filter(education_levels__contains=[level])
        serializer = SubjectSerializer(qs, many=True)
        return Response(serializer.data)
    ...
    def for_grade(self, request):
        ...
        if not grade_id:
            return Response({"error": "grade_id parameter required"}, status=400)
        subjects = self.get_queryset().filter(grade_levels__id=grade_id)
        serializer = SubjectSerializer(subjects, many=True)
        return Response(serializer.data)
    ...
    def search_suggestions(self, request):
        ...
        qs = self.get_queryset().filter(Q(name__icontains=q) | Q(code__icontains=q))[
            :10
        ]
    ...
    def statistics(self, request):
        ...
        data = {
            ...
            "with_practicals": qs.filter(has_practical=True).count(),
            "cross_cutting": qs.filter(is_cross_cutting=True).count(),
        }
        return Response(data)

    @action(detail=True, methods=["post"])
    def check_availability(self, request, pk=None):
        # Placeholder: example implementation could check timetable/assignments
        subject = get_object_or_404(Subject, pk=pk)
        available = not ClassroomTeacherAssignment.objects.filter(
    ...
    def prerequisites(self, request, pk=None):
        subject = get_object_or_404(Subject, pk=pk)
        serializer = SubjectListSerializer(subject.prerequisites.all(), many=True)
        return Response(serializer.data)

    @action(detail=True, methods=["get"])
    def education_levels(self, request, pk=None):
        subject = get_object_or_404(Subject, pk=pk)
        return Response({"education_levels": subject.education_levels})
...
class ClassroomViewSet(AutoSectionFilterMixin, viewsets.ModelViewSet):
    ...
    def get_queryset(self):
        """
        Filter classrooms based on user's role, section access and education level.
        Preserve AutoSectionFilterMixin behavior by starting from super().
        """
        base_qs = (
            super()
            .get_queryset()
        ...
        # Super Admin sees everything
        if getattr(user, "role", None) == "superadmin" or getattr(
            user, "is_superuser", False
        ):
            return base_qs

        try:
            # Apply section filtering provided by mixin (should consider user's tenant/sections)
            filtered_queryset = self.filter_classrooms_by_section_access(base_qs)

            # Separate nursery/primary vs secondary so we can prefetch appropriately
            nursery_primary_qs = filtered_queryset.filter(
                section__grade_level__education_level__in=["NURSERY", "PRIMARY"]
            ).select_related("class_teacher__user")

            secondary_qs = filtered_queryset.filter(
                section__grade_level__education_level__in=[
                    "JUNIOR_SECONDARY",
                    "SENIOR_SECONDARY",
                ]
            ).select_related("subject_teachers__user")

            # Combine (use union if DB supports it; fallback to chain via ids)
            try:
                combined = nursery_primary_qs.union(secondary_qs, all=True)
            except Exception:
                # Fallback: merge IDs
                ids = list(nursery_primary_qs.values_list("id", flat=True)) + list(
                    secondary_qs.values_list("id", flat=True)
                )
                combined = base_qs.filter(id__in=ids)

            return combined.order_by("section__grade_level__order", "name")
        except Exception as e:
            logger.error(
                f"Section filtering error for user {getattr(user, 'username', 'unknown')}: {str(e)}"
            )
            return base_qs.none()
    ...
    def students(self, request, pk=None):
        ...
        try:
            ...
            logger.debug(
                "Fetching students for classroom: %s (ID: %s)",
                classroom.name,
                classroom.id,
            )
            ...
            logger.exception("Error fetching classroom students")
            return Response(
                {"error": str(e)}, status=status.HTTP_500_INTERNAL_SERVER_ERROR
            )
    ...
    def remove_teacher(self, request, pk=None):
        ...
        try:
            ...
            logger.exception("Error removing teacher assignment")
            return Response(
                {"error": str(e)}, status=status.HTTP_500_INTERNAL_SERVER_ERROR
            )
    ...
    def unenroll_student(self, request, pk=None):
        ...
        try:
            ...
            logger.exception("Error unenrolling student")
            return Response(
                {"error": str(e)}, status=status.HTTP_500_INTERNAL_SERVER_ERROR
            )
...
class ClassroomTeacherAssignmentViewSet(AutoSectionFilterMixin, viewsets.ModelViewSet):
    ...
    def get_queryset(self):
        # Preserve mixin filtering and eager load related fields
        return (
            super()
            .get_queryset()
            .select_related("classroom", "teacher__user", "subject")
            .filter(is_active=True)
        )
        ...
        return Response(
            {
                ...
                "total_teachers": qs.values("teacher").distinct().count(),
                "total_classrooms": qs.values("classroom").distinct().count(),
                "total_subjects": qs.values("subject").distinct().count(),
            }
        )
...
class StudentEnrollmentViewSet(AutoSectionFilterMixin, viewsets.ModelViewSet):
    ...
    def statistics(self, request):
        ...
        secondary_enrollments = qs.filter(
            classroom__section__grade_level__education_level__in=[
                "JUNIOR_SECONDARY",
                "SENIOR_SECONDARY",
            ]
        ).count()
...
class ClassScheduleViewSet(AutoSectionFilterMixin, viewsets.ModelViewSet):
    ...
    @action(detail=False, methods=["get"])
    def conflicts(self, request):
        # naive conflict detection - could be expanded with better logic
        qs = self.get_queryset()
        # Example: detect same classroom, same timeslot multiple entries (requires timeslot fields)
        return Response({"message": "Conflict detection not implemented"}, status=501)

    @action(detail=False, methods=["get"])
    def daily_schedule(self, request):
        date = request.query_params.get("date")
        return Response(
            {"message": "Daily schedule endpoint not implemented"}, status=501
        )
    ...
    def weekly_schedule(self, request):
        ...
        return Response(
            {"message": "Weekly schedule endpoint not implemented"}, status=501
        )
...
# ================================
# Quick search / comparison / cache utilities
# ================================
class SubjectQuickSearchView(APIView):
    """
    Lightweight search endpoint for autocomplete and quick lookups.
            ...
            comparison_data.append(
                {
                    ...
                    "category": {
                        "code": subject.category,
                        "name": subject.get_category_display(),
                        "icon": getattr(subject, "category_icon", None)
                        or subject.get_category_display(),
                    },
                    "education_levels": {
                        "codes": subject.education_levels,
                        "display": getattr(
                    ...
                    "academic_info": {
                        ...
                        "is_cross_cutting": subject.is_cross_cutting,
                        "credit_hours": subject.credit_hours,
                        "practical_hours": getattr(subject, "practical_hours", 0),
                        "total_weekly_hours": getattr(
                            subject, "total_weekly_hours", None
                        ),
                        "pass_mark": getattr(subject, "pass_mark", None),
                    },
                    "practical_requirements": {
                        "has_practical": subject.has_practical,
                        "requires_lab": getattr(subject, "requires_lab", False),
                        "requires_special_equipment": getattr(
                            subject, "requires_special_equipment", False
                        ),
                        "equipment_notes": getattr(subject, "equipment_notes", None),
                    },
                    "teaching_requirements": {
                        "requires_specialist_teacher": subject.requires_specialist_teacher,
                    },
                    "assessment": {
                        "has_continuous_assessment": getattr(
                            subject, "has_continuous_assessment", False
                        ),
                        "has_final_exam": getattr(subject, "has_final_exam", True),
                    },
                    "prerequisites": {
                        "count": subject.prerequisites.count(),
                        "subjects": [
                            {
                                "id": prereq.id,
                                "name": getattr(prereq, "display_name", prereq.name),
                                "code": prereq.code,
                            }
                            for prereq in subject.prerequisites.all()
                        ],
                    ...
                    "special_attributes": {
                        "is_activity_based": subject.is_activity_based,
                        "nursery_levels": (
                            getattr(subject, "nursery_levels_display", None)
                            if getattr(subject, "is_nursery_subject", False)
                            else None
                        ),
                        "ss_subject_type": (
                            getattr(
                                subject, "get_ss_subject_type_display", lambda: None
                            )()
                            if getattr(subject, "ss_subject_type", None)
                            else None
                        ),
                    },
                }
        ...
        return Response(
            {
                ...
                "summary": {
                    "total_credit_hours": sum(
                        getattr(s, "credit_hours", 0) for s in subjects
                    ),
                    "total_practical_hours": sum(
                        getattr(s, "practical_hours", 0) for s in subjects
                    ),
                    "subjects_with_practicals": sum(
                        1 for s in subjects if getattr(s, "has_practical", False)
                    ),
                    "compulsory_subjects": sum(1 for s in subjects if s.is_compulsory),
                    "cross_cutting_subjects": sum(
                        1 for s in subjects if s.is_cross_cutting
...
def clear_subject_caches():
    ...
    try:
        ...
        logger.exception("Error clearing subject caches")
        return False
...
def system_info(request):
    try:
        ...
        logger.exception("System info endpoint failed")
        return Response(
            {
                "error": "Failed to retrieve system information",
                "timestamp": timezone.now().isoformat(),
