    ...
    def put(self, request):
        ...
        try:
            ...
            if serializer.is_valid():
                ...
                # Broadcast update event (if you have websockets)
...
# ============================================================================
# FILE UPLOAD VIEWS (CRITICAL FIX)
# ============================================================================
...
# ============================================================================
# DEPRECATED UPLOAD VIEWS (Keep for backward compatibility)
# ============================================================================
...
@permission_classes([IsAuthenticated, IsAdminUser])
def test_payment_gateway(request, gateway):
    """Test payment gateway connection with better validation"""
    try:
        credentials = request.data
        gateway = gateway.lower()

        if gateway == "paystack":
            public_key = credentials.get("publicKey", "").strip()
            secret_key = credentials.get("secretKey", "").strip()
            ...
            if not public_key.startswith("pk_"):
                return Response(
                    {"success": False, "message": "Invalid Paystack public key format"},
                    status=status.HTTP_400_BAD_REQUEST,
                )

            if not secret_key.startswith("sk_"):
                return Response(
                    {"success": False, "message": "Invalid Paystack secret key format"},
                    status=status.HTTP_400_BAD_REQUEST,
                )
            ...
            publishable_key = credentials.get("publishableKey", "").strip()
            secret_key = credentials.get("secretKey", "").strip()
            ...
            # TODO: Add actual Stripe API verification here
            return Response(
                {"success": True, "message": "Stripe connection test successful!"}
            )
            ...
            # TODO: Add actual Flutterwave API verification here
            return Response(
                {"success": True, "message": "Flutterwave connection test successful!"}
            )
        ...
        logger.error(f"Payment gateway test failed: {str(e)}", exc_info=True)
        return Response(
            {
                "success": False,
                "message": f"Failed to test {gateway} connection: {str(e)}",
...
class CommunicationSettingsDetail(APIView):
    ...
    def get(self, request):
        ...
        try:
            ...
            logger.error(
                f"Failed to fetch communication settings: {str(e)}", exc_info=True
            )
            return Response(
                {"error": f"Failed to fetch communication settings: {str(e)}"},
                status=status.HTTP_500_INTERNAL_SERVER_ERROR,
    ...
    def put(self, request):
        ...
        try:
            ...
            logger.error(
                f"Failed to update communication settings: {str(e)}", exc_info=True
            )
            return Response(
                {"error": f"Failed to update communication settings: {str(e)}"},
                status=status.HTTP_500_INTERNAL_SERVER_ERROR,
...
class PermissionViewSet(viewsets.ModelViewSet):
    """ViewSet for managing permissions with better filtering"""
    ...
    def get_queryset(self):
        ...
        # Apply filters
        for param in ["module", "permission_type", "section"]:
...
class RoleViewSet(viewsets.ModelViewSet):
    """ViewSet for managing roles with enhanced functionality"""
