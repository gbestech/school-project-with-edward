...
from .permissions import HasSettingsPermission, HasSettingsPermissionOrReadOnly, PublicReadOnly
...
import json
import os
from .models import SchoolSettings, SchoolAnnouncement, CommunicationSettings, Permission, Role, UserRole
...
class CommunicationSettingsDetail(APIView):
    ...
    def put(self, request):
        ...
        try:
            ...
            serializer = CommunicationSettingsSerializer(settings, data=data, partial=True)
            if serializer.is_valid():
    ...
    try:
        ...
        try:
            ...
            if response.status_code == 200:
                ...
                settings.save(update_fields=['brevo_api_key', 'brevo_sender_email', 'brevo_configured'])
...
def test_twilio_connection(request):
    ...
    try:
        ...
        if not account_sid or not auth_token:
            return Response(
                {'success': False, 'message': 'Twilio Account SID and Auth Token are required'},
                status=status.HTTP_400_BAD_REQUEST
            )
        ...
        if success:
            ...
            settings.save(update_fields=['twilio_account_sid', 'twilio_auth_token', 'twilio_phone_number', 'twilio_configured'])
        ...
        return Response(
            {'success': False, 'message': f'Failed to test Twilio connection: {str(e)}'},
            status=status.HTTP_500_INTERNAL_SERVER_ERROR
        )
...
def send_test_email(request):
    ...
    try:
        ...
        if not settings or not settings.brevo_configured:
            return Response(
                {'success': False, 'message': 'Brevo is not configured. Please configure Brevo first.'},
                status=status.HTTP_400_BAD_REQUEST
            )
        ...
        if status_code in [200, 201]:
            ...
            return Response({
                'success': False,
                'message': f'Failed to send test email: {response}'
            })
...
def send_test_sms(request):
    ...
    try:
        ...
        if not settings or not settings.twilio_configured:
            return Response(
                {'success': False, 'message': 'Twilio is not configured. Please configure Twilio first.'},
                status=status.HTTP_400_BAD_REQUEST
            )
    ...
    def bulk_create(self, request):
        ...
        try:
            ...
            return Response({
                ...
                'permissions': PermissionSerializer(created_permissions, many=True).data
            })
            ...
            for user_role in user_roles:
                if not user_role.is_expired():
                    user_data.append({
                        ...
                        'assigned_by': user_role.assigned_by.full_name if user_role.assigned_by else None,
            ...
            for user_role in user_roles:
                ...
                for permission in user_role.role.permissions.all():
                    if permission.granted:
                        effective_permissions[permission.module][permission.permission_type] = True
                        role_data['permissions'][f"{permission.module}_{permission.permission_type}"] = True
