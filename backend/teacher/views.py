...
#     def _safe_super_get_queryset(self, default_queryset):
#         """
#         Try calling super().get_queryset() — but if parent doesn't implement
#         get_queryset or raises AssertionError (DRF requires queryset), return default_queryset.
#         This avoids crashes when mixins/parents expect a queryset attribute.
#         """
#         try:
#             parent_get = super(TeacherViewSet, self).get_queryset
#         except Exception:
#             parent_get = None
...
#             try:
#                 qs = parent_get()
#                 # Ensure we return a queryset-like object; if parent raised AssertionError, it will be caught.
#                 return qs
#             except (AttributeError, AssertionError, TypeError):
#                 return default_queryset
#             except Exception:
#                 # Unexpected error from parent's get_queryset — fall back safely but log
#                 return default_queryset
#         else:
#             return default_queryset
...
#             admin_section = getattr(user, "section", None)
...
#         # Apply section filtering using a safe super call
#         default_qs = Teacher.objects.select_related("user").all()
#         queryset = self._safe_super_get_queryset(default_qs)
...
#         elif user.is_staff and getattr(user, "is_section_admin", False):
#             education_levels = self._get_section_education_levels(user)
...
#             print(
#                 f"Section admin {user.username} ({getattr(user,'role',None)}) — "
...
#     def _safe_super_get_queryset(self, default_queryset):
#         try:
#             parent_get = super(AssignmentRequestViewSet, self).get_queryset
#         except Exception:
#             parent_get = None
...
#     def get_queryset(self):
#         # safe default
#         default_qs = AssignmentRequest.objects.all()
#         queryset = self._safe_super_get_queryset(default_qs)
...
#     def _safe_super_get_queryset(self, default_queryset):
#         try:
#             parent_get = super(TeacherScheduleViewSet, self).get_queryset
#         except Exception:
#             parent_get = None
...
#     def get_queryset(self):
#         # safe default
#         default_qs = TeacherSchedule.objects.all()
#         queryset = self._safe_super_get_queryset(default_qs)
