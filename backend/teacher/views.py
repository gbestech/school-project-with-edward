...
class TeacherScheduleViewSet(viewsets.ModelViewSet):
    ...
    def weekly_schedule(self, request):
        ...
        for day in days:
            weekly_schedule[day] = schedules.filter(day_of_week=day).order_by('start_time')
...
class AssignmentManagementViewSet(viewsets.ViewSet):
    ...
    def available_grade_levels(self, request):
        ...
        return Response({
            'grade_levels': [
                {'id': grade.id, 'name': grade.name, 'education_level': grade.education_level}
    ...
    def available_sections(self, request):
        ...
        return Response({
            'sections': [
                {'id': section.id, 'name': section.name, 'grade_level': section.grade_level.name}
    ...
    def teacher_assignments_summary(self, request):
        ...
        subject_assignments = teacher.teacher_assignments.values('subject').distinct().count()
        class_assignments = teacher.teacher_assignments.values('grade_level', 'section').distinct().count()
        total_students = 0  # This would need to be calculated based on actual student enrollment
