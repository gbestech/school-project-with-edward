...
class Subject(models.Model):
    ...
    def clean(self):
        ...
        if self.nursery_levels and "NURSERY" not in self.education_levels:
            raise ValidationError("Nursery levels can only be set for NURSERY education level")

        if self.ss_subject_type and "SENIOR_SECONDARY" not in self.education_levels:
            raise ValidationError("SS subject type can only be set for SENIOR_SECONDARY education level")
...
class SchoolStreamConfiguration(models.Model):
    ...
    school_id = models.IntegerField(
        default=1,
        help_text="School ID (default: 1 for single-school systems)"
    )
    ...
    min_subjects_required = models.PositiveIntegerField(
        default=1,
        help_text="Minimum number of subjects required from this category"
    )
    
    max_subjects_allowed = models.PositiveIntegerField(
        default=5,
        help_text="Maximum number of subjects allowed from this category"
    )
    
    # Whether this category is compulsory for the stream
    is_compulsory = models.BooleanField(
        default=True,
        help_text="Whether students must take subjects from this category"
    )
    
    # Priority/order for display
    display_order = models.PositiveIntegerField(
        default=0,
        help_text="Display order for this configuration"
    )
    ...
    def __str__(self):
        return f"{self.school.name} - {self.stream.name} - {self.get_subject_role_display()}"
...
class SchoolStreamSubjectAssignment(models.Model):
    ...
    subject = models.ForeignKey(
        Subject,  # Now Subject is defined above, so this reference works
        on_delete=models.CASCADE,
    ...
    is_compulsory = models.BooleanField(
        default=False,
        help_text="Whether this specific subject is compulsory"
    )
    
    # Credit weight for the subject
    credit_weight = models.PositiveIntegerField(
        default=1,
        help_text="Credit weight for this subject in the stream"
    )
