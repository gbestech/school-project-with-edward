...
from .models import (
    ...
    StudentResult, StudentTermResult, ResultSheet,
    AssessmentScore, ResultComment,
...
class ScoringConfigurationSerializer(serializers.ModelSerializer):
    ...
    total_ca_max_score = serializers.DecimalField(source='ca_total_max_score', read_only=True, max_digits=5, decimal_places=2)
    
    # Add frontend field names for compatibility
    first_test_max_score = serializers.DecimalField(source='test1_max_score', read_only=True, max_digits=5, decimal_places=2)
    second_test_max_score = serializers.DecimalField(source='test2_max_score', read_only=True, max_digits=5, decimal_places=2)
    third_test_max_score = serializers.DecimalField(source='test3_max_score', read_only=True, max_digits=5, decimal_places=2)
    
    class Meta:
        model = ScoringConfiguration
        fields = [
            'id', 'name', 'education_level', 'education_level_display', 'result_type',
            'description', 'test1_max_score', 'test2_max_score',
            'test3_max_score', 'first_test_max_score', 'second_test_max_score', 'third_test_max_score',
            'continuous_assessment_max_score', 'take_home_test_max_score',
            'appearance_max_score', 'practical_max_score', 'project_max_score', 'note_copying_max_score',
            'exam_max_score', 'total_max_score',
            'ca_weight_percentage', 'exam_weight_percentage',
            'total_ca_max_score', 'is_active', 'is_default',
...
class ScoringConfigurationCreateUpdateSerializer(serializers.ModelSerializer):
    ...
    first_test_max_score = serializers.DecimalField(source='test1_max_score', max_digits=5, decimal_places=2, coerce_to_string=False, required=False, allow_null=True)
    second_test_max_score = serializers.DecimalField(source='test2_max_score', max_digits=5, decimal_places=2, coerce_to_string=False, required=False, allow_null=True)
    third_test_max_score = serializers.DecimalField(source='test3_max_score', max_digits=5, decimal_places=2, coerce_to_string=False, required=False, allow_null=True)
    
    class Meta:
        model = ScoringConfiguration
        fields = [
            'name', 'education_level', 'result_type', 'description',
            'first_test_max_score', 'second_test_max_score', 'third_test_max_score',
            'continuous_assessment_max_score', 'take_home_test_max_score',
            'appearance_max_score', 'practical_max_score', 'project_max_score', 'note_copying_max_score',
            'exam_max_score', 'total_max_score',
            'ca_weight_percentage', 'exam_weight_percentage',
    ...
    def validate(self, data):
        ...
        if education_level in ["JUNIOR_SECONDARY", "PRIMARY"]:
            ...
            for field in required_fields:
                if field not in data or data[field] is None:
                    raise serializers.ValidationError({
                        field: ["This field is required for Junior Secondary and Primary education levels."]
                    })
            ...
            for field in required_fields:
                if field not in data or data[field] is None:
                    raise serializers.ValidationError({
                        field: ["This field is required for Senior Secondary education level."]
                    })
            ...
            if 'total_max_score' not in data or data['total_max_score'] is None:
                raise serializers.ValidationError({
                    'total_max_score': ["Max Mark Obtainable is required for Nursery education level."]
                })
        ...
        if result_type == 'TERMLY' and education_level != 'NURSERY':
            ...
            if ca_weight + exam_weight != 100:
                raise serializers.ValidationError({
                    'non_field_errors': ["CA weight percentage and exam weight percentage must sum to 100"]
                })
        ...
        print(f"DEBUG: first_test_max_score = {data.get('first_test_max_score', 0)}")
        print(f"DEBUG: second_test_max_score = {data.get('second_test_max_score', 0)}")
        print(f"DEBUG: third_test_max_score = {data.get('third_test_max_score', 0)}")
        print(f"DEBUG: exam_max_score = {data.get('exam_max_score', 0)}")
        print(f"DEBUG: total_max_score = {data.get('total_max_score', 0)}")
        ...
        if result_type == 'TERMLY':
            if education_level in ["JUNIOR_SECONDARY", "PRIMARY"]:
                ...
                print(f"DEBUG: Primary/Junior Secondary components:")
                print(f"  CA Score: {ca_score}")
                print(f"  Take Home: {take_home}")
                print(f"  Appearance: {appearance}")
                print(f"  Practical: {practical}")
                print(f"  Project: {project}")
                print(f"  Note Copying: {note_copying}")
                print(f"  Exam: {exam}")
                
                expected_total = ca_score + take_home + appearance + practical + project + note_copying + exam
            elif education_level == "SENIOR_SECONDARY":
                ...
                first_test = data.get('first_test_max_score', data.get('test1_max_score', 0))
                second_test = data.get('second_test_max_score', data.get('test2_max_score', 0))
                third_test = data.get('third_test_max_score', data.get('test3_max_score', 0))
            ...
            if expected_total != total_max_score:
                raise serializers.ValidationError({
                    'total_max_score': [f"Total max score must equal sum of components ({expected_total})"]
                })
    ...
    def create(self, validated_data):
        ...
        decimal_fields = [
            'test1_max_score', 'test2_max_score', 'test3_max_score',
            'continuous_assessment_max_score', 'take_home_test_max_score',
            'appearance_max_score', 'practical_max_score', 'project_max_score', 'note_copying_max_score',
            'exam_max_score', 'total_max_score', 'ca_weight_percentage', 'exam_weight_percentage'
        ]
...
class SeniorSecondaryResultCreateUpdateSerializer(serializers.ModelSerializer):
    ...
    def validate(self, data):
        ...
        if data.get('second_test_score', 0) > 10:
            raise serializers.ValidationError("Second test score cannot exceed 10 marks")
...
class SeniorSecondarySessionResultCreateUpdateSerializer(serializers.ModelSerializer):
    ...
    def validate(self, data):
        ...
        if first_term > 100:
            raise serializers.ValidationError("First term score cannot exceed 100 marks")
        if second_term > 100:
            raise serializers.ValidationError("Second term score cannot exceed 100 marks")
        if third_term > 100:
            raise serializers.ValidationError("Third term score cannot exceed 100 marks")
